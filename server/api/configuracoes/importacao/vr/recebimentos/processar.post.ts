...
import { buildVrRecebimentosFromParsedFiles } from '../../../../../utils/vrLayout16ap'
...
export default defineEventHandler(async (event) => {
  ...
  const resultado = buildVrRecebimentosFromParsedFiles({
    files: arquivosLidos,
    empresa,
    ec
  })
  ...
  if (resultado.registros.length === 0) {
    throw createError({
      statusCode: 422,
      statusMessage: 'Os arquivos VR selecionados nao possuem liquidações (L/A) nem ajustes (A) para importar em recebimentos.'
    })
  }

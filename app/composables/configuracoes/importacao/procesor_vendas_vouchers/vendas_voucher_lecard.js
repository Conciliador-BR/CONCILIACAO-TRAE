import { useTaxOperations } from '~/composables/configuracoes/importacao/Envio_vendas/calculo_previsao_pgto/useTaxOperations.js'
...
export const useProcessorVendasVoucherLecard = () => {
  ...
  const formatarData = (valor) => {
    ...
    const mAny = s.match(/(\d{1,2})\/(\d{1,2})\/(\d{4})/)
    if (mAny) {
      const dia = mAny[1]
      const mes = mAny[2]
      const ano = mAny[3]
  ...
  const detectarLinhaCabecalho = (matriz, maxLinhas=25) => {
    const candidatos = ['COMPR','COMPR.','COMPROVANTE','AUTORIZACAO','AUTORIZAÇÃO','DATA','VALOR','PRODUTO','MODALIDADE']
  ...
  const processarDados = async (dados, contexto) => {
    ...
    const ALIASES = {
      nsu: ['COMPR','COMPR.','COMPROVANTE','AUTORIZACAO','AUTORIZAÇÃO','Nº AUTORIZACAO','Nº AUTORIZAÇÃO'],
    ...
    for (let i = inicio; i < dados.length; i++) {
      ...
      if (normRow.some(c => 
        c.includes('TOTAL DO DIA') ||
        c.includes('TOTAL MOVIMENTO') ||
        c.includes('COMPRAS') ||
        c.includes('RESUMO') ||
        c.includes('TOTAL') ||
        c.includes('PAGINA')
      ...
      const item = {
        adquirente: (operadora || 'LECARD').toString().trim().toUpperCase(),
        nsu: '',
        data_venda: null,
        data_venda_text: '',
        previsao_pgto: null,
      ...
      for (const [idxStr, campoDb] of Object.entries(colIndexParaCampo)) {
        ...
        switch (campoDb) {
          ...
          case 'produto': item.modalidade = 'Rede'; break
          default: break
        }
      }
      if (!item.modalidade || String(item.modalidade).trim() === '') item.modalidade = 'Rede'
      if ((!item.nsu || String(item.nsu).trim() === '') && (!item.data_venda && !item.data_venda_text) && (Number(item.valor_bruto || 0) === 0) && (Number(item.valor_liquido || 0) === 0)) {
        continue
      }

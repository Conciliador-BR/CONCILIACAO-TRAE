...
export const useVendasOperadoraAzulzinha = () => {
  ...
  const processarDados = (dados, nomeEmpresa) => {
    ...
    const ALIASES = {
      ...
      valor_bruto: ['VALOR BRUTO DA PARCELA'],
      valor_liquido: ['VALOR LIQUIDO DA PARCELA/TRANSACAO', 'VALOR LÍQUIDO DA PARCELA/TRANSAÇÃO'],
      taxa_mdr: ['PERCENTUAL DA TAXA (MDR)'],
      despesa_mdr: ['VALOR DA TAXA (MDR)'],
      numero_parcelas: ['NÚMERO DE PARCELAS', 'NUMERO DE PARCELAS', 'PARCELAS'],
      bandeira: ['BANDEIRA', 'ARRANJO'],
      status: ['STATUS DA VENDA', 'STATUS'],
      prazo_recebimento: ['DATA PREVISTA DE PAGAMENTO DA VENDA']
    }
        ...
        for (const [idxStr, campoDb] of Object.entries(colIndexParaCampo)) {
          ...
          switch (campoDb) {
            ...
            case 'prazo_recebimento':
              r[campoDb === 'prazo_recebimento' ? 'previsao_pgto' : 'data_venda'] = formatarData(valor)
              break
            case 'valor_bruto':
            case 'valor_liquido':
            case 'despesa_mdr':

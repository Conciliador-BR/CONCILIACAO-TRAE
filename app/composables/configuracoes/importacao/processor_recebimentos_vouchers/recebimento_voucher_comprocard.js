...
export const useProcessorRecebimentoVoucherComprocard = () => {
  ...
  const processarRecebimentos = async (matriz, contexto) => {
    ...
    const { idx: headerRowIdx, headersNorm } = detectarLinhaCabecalho(matriz, [
      'NUM. AUTORIZAÇÃO','NUM AUTORIZAÇÃO','NUM AUTORIZACAO','NUMERO AUTORIZACAO','NUMERO AUTORIZAÇÃO','Nº DA AUTORIZAÇÃO','N° DA AUTORIZAÇÃO','AUTORIZACAO','AUTORIZAÇÃO',
      'DATA DA VENDA','DATA VENDA',
      'TIPO CARTAO','TIPO CARTÃO','TIPO DO CARTAO','TIPO DO CARTÃO','MODALIDADE','PRODUTO',
      'VALOR DA VENDA','VALOR VENDA','VL VENDA','VL. VENDA','VLR VENDA','VALOR BRUTO','VALOR (R$)','VALOR'
    ])
    const ALIASES = {
      nsu: ['NUM. AUTORIZAÇÃO','NUM AUTORIZAÇÃO','NUM AUTORIZACAO','NUMERO AUTORIZACAO','NUMERO AUTORIZAÇÃO','Nº DA AUTORIZAÇÃO','N° DA AUTORIZAÇÃO','AUTORIZACAO','AUTORIZAÇÃO'],
      data_venda: ['DATA DA VENDA','DATA VENDA'],
      modalidade: ['TIPO CARTAO','TIPO CARTÃO','TIPO DO CARTAO','TIPO DO CARTÃO','MODALIDADE','PRODUTO'],
      valor_bruto: ['VALOR DA VENDA','VALOR VENDA','VL VENDA','VL. VENDA','VLR VENDA','VALOR BRUTO','VALOR (R$)','VALOR']
    }
    ...
    for (let i = inicio; i < matriz.length; i++) {
      ...
      if (Number(item.valor_bruto || 0) === 0) {
        const valores = []
        for (const v of row) {
          if (typeof v === 'number') {
            const n = formatarValor(v)
            if (Number.isFinite(n) && n > 0) valores.push(n)
          } else {
            const s = String(v || '').trim()
            if (/^R\$\s*[\d\.]+,\d{2}$/.test(s) || /[\d\.]+,\d{2}/.test(s)) {
              const n = formatarValor(s)
              if (Number.isFinite(n) && n > 0) valores.push(n)
            }
          }
        }
        if (valores.length >= 1) item.valor_bruto = valores[0]
      }
      out.push(item)
    }
    return out

...
export const useVendasOperadoraCielo = () => {
  ...
  const processarDados = (dados, nomeEmpresa) => {
    ...
    const ALIASES = {
      ...
      taxa_mdr: ['TOTAL DE TAXAS','TOTAL TAXAS','% TAXAS'],
      ...
      previsao_pgto: ['DATA PREVISTA DO PAGAMENTO','PREVISAO PGTO','DATA PAGAMENTO PREVISTA']
    }
      ...
      try {
        ...
        for (const [idxStr, campoDb] of Object.entries(colIndexParaCampo)) {
          ...
          switch (campoDb) {
            ...
            case 'status': r.status = valor != null ? String(valor).trim() : ''; break
  ...
  const detectarLinhaCabecalho = (matriz, maxLinhas=30) => {
    const candidatos = ['DATA DA VENDA','FORMA DE PAGAMENTO','NSU/DOC','VALOR BRUTO','VALOR LIQUIDO','TAXA/TARIFA','TOTAL DE TAXAS','QUANTIDADE TOTAL DE PARCELAS','BANDEIRA','STATUS DA VENDA','DATA PREVISTA DO PAGAMENTO']
    let melhorIdx = -1
    let melhorScore = -1

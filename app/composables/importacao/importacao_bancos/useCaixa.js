...
export const useCaixa = () => {
  ...
  const processarOFX = async (arquivo) => {
    ...
    try {
      ...
      const transacoesProcessadas = parseOFXCaixa(texto)
  ...
  const processarPDF = async (arquivo) => {
    ...
    transacoes.value = []
    try {
      const r = await processarPDFInterno(arquivo)
      if (r.sucesso) {
        transacoes.value = r.transacoes
      } else {
        erro.value = r.erro || 'Erro ao processar PDF da Caixa'
      }
      return r
    } catch (error) {
  ...
  const parseOFXCaixa = (textoOFX) => {
    ...
    try {
      ...
      throw new Error(`Erro ao processar OFX da Caixa: ${error.message}`)
    }
  }
  ...
  const parseTransacaoOFX = (textoTransacao, indice) => {
    ...
    const documento = extrairCampo('FITID') || extrairCampo('REFNUM') || `CAIXA-${indice}`

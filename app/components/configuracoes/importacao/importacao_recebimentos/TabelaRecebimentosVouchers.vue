<template>
  <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
    <div class="bg-gradient-to-r from-gray-50 to-white px-8 py-6 border-b border-gray-200">
      <h2 class="text-2xl font-bold text-gray-900">4. Recebimentos Processados (Vouchers)</h2>
      <p class="text-sm text-gray-600 mt-1">Pré-visualização dos recebimentos de vouchers antes do envio</p>
    </div>
    ...
    <div class="p-6 overflow-x-auto">
      <div class="flex flex-col gap-4 mb-4 xl:flex-row xl:items-center xl:justify-between">
        <div class="flex flex-wrap items-center gap-6">
          ...
          <div>
            <p class="text-xs text-gray-500">Recebimentos Líquidos</p>
        ...
        <thead class="bg-gray-50">
          <tr>
            ...
            <template v-if="modoVisualizacao === 'detalhado'">
              ...
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Data Recebimento</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Modalidade</th>
            </template>
            <template v-else>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Data Recebimento</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Data Venda(s)</th>
        ...
        <tbody class="bg-white divide-y divide-gray-100">
          <tr v-for="(v, idx) in rowsPagina" :key="idx" class="hover:bg-gray-50">
            ...
            <template v-if="modoVisualizacao === 'detalhado'">
              ...
              <td class="px-4 py-3 text-sm text-gray-700">{{ formatarDataExibicao(v.data_recebimento) }}</td>
          ...
          <tr v-if="registrosAtivos.length === 0">
            <td :colspan="modoVisualizacao === 'resumo' ? 11 : 10" class="px-4 py-6 text-center text-sm text-gray-500">Nenhum recebimento de voucher processado</td>
          </tr>
        </tbody>
      </table>
...
const recebimentosFormatados = computed(() => props.recebimentos.map(v => ({
  adquirente: (v.adquirente || '').toString().toUpperCase(),
  nsu: v.nsu || '',
  data_venda: v.data_venda || null,
  data_recebimento: v.data_recebimento || null,
  modalidade: v.modalidade || '',
  valor_bruto: toNumber(v.valor_bruto),
  despesa: toNumber(v.despesa),
  valor_liquido: toNumber(v.valor_liquido),
  empresa: v.empresa || '',
  ec: v.ec || ''
})))
...
const resumoPorDia = computed(() => {
  ...
  for (const item of recebimentosFormatados.value) {
    const chave = String(item.data_recebimento || item.data_venda || 'sem-data')
    ...
    if (item.data_venda) atual.datasVenda.add(String(item.data_venda))
  ...
  return Array.from(agrupado.values()).map((item) => ({
    ...
    data_venda: item.datasVenda.size === 1 ? Array.from(item.datasVenda)[0] : item.datasVenda.size > 1 ? 'Diversas' : null
  }))
})

const totalBruto = computed(() => recebimentosFormatados.value.reduce((sum, v) => sum + (v.valor_bruto || 0), 0))
const totalLiquido = computed(() => recebimentosFormatados.value.reduce((sum, v) => sum + (v.valor_liquido || 0), 0))
const totalDespesa = computed(() => recebimentosFormatados.value.reduce((sum, v) => sum + (v.despesa || 0), 0))

const pageSize = ref(10)
const currentPage = ref(1)
const registrosAtivos = computed(() => modoVisualizacao.value === 'resumo' ? resumoPorDia.value : recebimentosFormatados.value)
...
watch(recebimentosFormatados, () => { currentPage.value = 1 })

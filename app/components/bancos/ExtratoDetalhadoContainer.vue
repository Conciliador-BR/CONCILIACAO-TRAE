<template>
  <div class="flex-1 flex flex-col w-full">
    <div class="w-full max-w-none">
      <!-- Filtros -->
      <div class="mb-7">
      <div class="overflow-hidden rounded-[28px] border border-[#DCE7F3] bg-white shadow-[0_18px_40px_rgba(16,42,67,0.08)]">
        <div class="border-b border-[#E7EFF8] px-6 py-4">
            <h3 class="mb-4 text-xl font-bold text-[#102A43]">Filtros de Extrato</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select 
                  ...
                  class="w-full rounded-xl border border-[#BFD3EA] bg-white px-4 py-3 text-[#102A43] shadow-sm transition-all duration-200 focus:border-[#8bb5de] focus:outline-none focus:ring-2 focus:ring-[#8bb5de]/40"
                >
            ...
            <!-- Botão de Busca -->
            <div class="mt-6 border-t border-[#E7EFF8] pt-6">
              <button 
    ...
    <div v-if="!empresaSelecionada" class="flex-1 flex items-center justify-center">
      <div class="text-center">
        ...
        <h3 class="mb-2 text-lg font-medium text-slate-900">Nenhuma empresa selecionada</h3>
        <p class="text-slate-500">Selecione uma empresa para visualizar o extrato detalhado</p>
      </div>
    </div>
    ...
    <div v-else-if="loading" class="flex-1 flex items-center justify-center">
      <div class="text-center">
        ...
        <p class="text-slate-500">Carregando extrato detalhado...</p>
      </div>
    </div>
    ...
    <div v-else-if="error" class="flex-1 flex items-center justify-center">
      <div class="text-center">
        ...
        <h3 class="mb-2 text-lg font-medium text-slate-900">Erro ao carregar dados</h3>
        <p class="mb-4 text-slate-500">{{ error }}</p>
        <button 
          @click="buscarDados"
    ...
    <div v-else-if="!transacoes || transacoes.length === 0" class="flex-1 flex items-center justify-center">
      <div class="text-center">
        <div class="mb-4 text-6xl text-slate-300">📊</div>
        <h3 class="mb-2 text-lg font-medium text-slate-900">Nenhuma transação encontrada</h3>
        <p class="text-slate-500">Ajuste os filtros e tente novamente</p>
      </div>
    </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <nav class="flex space-x-8">
                <button
                  ...
                  :class="abaAtivaExtrato === 'todas' ? 'bg-gradient-to-r from-[#102a43] via-[#163a5a] to-[#1f4f77] text-white border-[#244b77] shadow-lg ring-2 ring-[#8bb5de] border-b-2' : 'border-transparent text-slate-500 hover:text-slate-800 hover:border-slate-300 hover:bg-slate-50'"
                >
                ...
                <button
                  ...
                  class="py-3 px-4 border-b-2 font-medium text-sm transition-colors duration-200 rounded-t-lg"
                  :class="abaAtivaExtrato === 'resumidas' ? 'bg-gradient-to-r from-[#102a43] via-[#163a5a] to-[#1f4f77] text-white border-[#244b77] shadow-lg ring-2 ring-[#8bb5de] border-b-2' : 'border-transparent text-slate-500 hover:text-slate-800 hover:border-slate-300 hover:bg-slate-50'"
                >
              ...
              <div class="text-sm">
                <label class="mr-2 text-slate-500">Mês:</label>
                <select
                  v-model="mesSelecionado"
                  class="rounded-lg border border-[#BFD3EA] bg-white px-3 py-2 text-sm text-[#102A43] shadow-sm transition-all duration-200 focus:border-[#8bb5de] focus:outline-none focus:ring-2 focus:ring-[#8bb5de]/40"
                >

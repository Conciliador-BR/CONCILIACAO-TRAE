    ...
    <div class="p-6 overflow-x-auto">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-100">
          ...
          <tr v-if="recebimentosFormatados.length === 0">
            <td colspan="10" class="px-4 py-6 text-center text-sm text-gray-500">Nenhum recebimento de voucher processado</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="flex items-center justify-between mt-4">
        <div class="text-sm text-gray-600">
          Mostrando {{ inicioIndice + 1 }}–{{ fimIndice }} de {{ recebimentosFormatados.length }}
        </div>
        <div class="flex items-center gap-2">
...
const totalPages = computed(() => Math.max(1, Math.ceil(recebimentosFormatados.value.length / pageSize.value)))
const inicioIndice = computed(() => (currentPage.value - 1) * pageSize.value)
const fimIndice = computed(() => Math.min(recebimentosFormatados.value.length, inicioIndice.value + pageSize.value))
const rowsPagina = computed(() => recebimentosFormatados.value.slice(inicioIndice.value, fimIndice.value))

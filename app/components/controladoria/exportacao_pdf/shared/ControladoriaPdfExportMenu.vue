<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-[1200]">
      <div class="absolute inset-0 bg-slate-950/20 backdrop-blur-[2px]" @click="$emit('close')"></div>

      <div class="absolute right-6 top-24 w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
        <div class="flex items-start justify-between gap-4">
          <div>
            <p class="text-sm font-semibold text-slate-900">Exportar PDF</p>
            <p class="mt-1 text-xs text-slate-500">Selecione as pages que deseja gerar. Um PDF sera aberto por page selecionada.</p>
          </div>
        ...
        <div class="mt-5 flex items-center justify-end gap-3">
          ...
          >
            ...
            {{ loading ? 'Preparando exportacao...' : 'Gerar PDF(s)' }}
          </button>
        </div>
      </div>
    </div>
...
defineEmits(['close', 'confirm', 'toggle-option', 'toggle-all'])
</script>

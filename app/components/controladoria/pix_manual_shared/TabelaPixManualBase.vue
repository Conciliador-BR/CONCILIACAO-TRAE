<template>
  <div class="bg-white rounded-2xl shadow-xl border border-gray-200 mt-6 overflow-hidden">
    ...
    <div v-if="successMessage" class="bg-green-50 text-green-700 px-6 py-3 text-sm border-b border-green-100 flex justify-between items-center">
      ...
      <button @click="limparSuccess" class="text-green-800 hover:text-green-900">&times;</button>
    </div>
      ...
      <table class="w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-100">
          <template v-for="(linha, index) in linhasExibidas" :key="linha._row_key">
            <tr class="hover:bg-blue-50 transition-colors duration-200 group">
              ...
              <td
                v-for="column in leadingColumns"
                :key="`${linha._row_key}-${column.key}`"
                class="px-6 py-5 whitespace-nowrap text-right text-sm font-medium"
              >
                <span class="text-gray-400">{{ formatCurrency(linha[column.key]) }}</span>
              </td>
              ...
              <td v-if="canManageManualTables" class="col-acoes-pdf px-6 py-5 whitespace-nowrap text-center">
                <div class="inline-flex items-center gap-2">
                  <button
                    @click="adicionarLinhaAtual(index)"
                    :disabled="!empresaSelecionada || loading"
            ...
            <tr v-if="activeObservationIndex === index || temObservacao(linha)" class="bg-slate-50/80">
              <td :colspan="columnCount" class="px-6 pb-5 pt-0">
                ...
                <div v-else class="rounded-xl border border-slate-200/80 bg-slate-50 px-4 py-3">
        ...
        <tfoot class="bg-gradient-to-r from-blue-600 to-blue-700 text-white">
          <tr class="font-bold">
            ...
            >
              {{ formatCurrency(totais[column.key]) }}
            </td>
...
const props = defineProps({
  ...
  variant: {
    type: String,
    required: true,
    validator: (value) => ['vendas', 'recebimentos'].includes(value)
  },
  title: {
    type: String,
    required: true
...
const leadingColumns = computed(() => {
  if (props.variant === 'vendas') {
    return [
      { key: 'debito', label: 'Débito' },
      { key: 'credito', label: 'Crédito' },
      { key: 'credito3x', label: 'Crédito 3x' },
      { key: 'credito4x6x', label: 'Crédito 4x-6x' }
    ]
  }
...
const linhasExibidas = computed(() => {
  if (!props.empresaSelecionada) return []
  return unref(props.controller?.pixData) || []
})

const loading = computed(() => Boolean(unref(props.controller?.loading)))
const error = computed(() => unref(props.controller?.error))
const successMessage = computed(() => unref(props.controller?.successMessage))
const columnCount = computed(() => 5 + leadingColumns.value.length + (canManageManualTables.value ? 2 : 0))

const totais = computed(() => {
  const numericFields = [
    ...leadingColumns.value.map((column) => column.key),
    'pix',
    'despesa_mdr',
    'valor_bruto',
    'valor_liquido'
  ...
  return linhasExibidas.value.reduce((acc, linha) => {
    numericFields.forEach((field) => {
      acc[field] += Number(linha?.[field] || 0)
    })
    return acc
...
const onInputPix = (linha, event) => {
  ...
  linha.pix = props.controller?.parseBRL?.(raw) || 0
  props.controller?.recalcularLinha?.(linha)
}
...
const onInputMdr = (linha, event) => {
  ...
  linha.despesa_mdr = props.controller?.parseBRL?.(raw) || 0
  props.controller?.recalcularLinha?.(linha)
}
...
const linhaTemAlteracoes = (linha) => {
  ...
  return Boolean(nomeAtual) && (
    ...
    Number(linha._delta_bruto || 0) !== 0 ||
    Number(linha._delta_mdr || 0) !== 0 ||
    observacaoAtual !== observacaoOriginal
  )
}

const adicionarLinhaAtual = (index) => {
  props.controller?.adicionarLinha?.(index)
}

const enviarLinhaAtual = async (linha) => {
  await props.controller?.enviarLinha?.(linha)
}

const removerLinhaAtual = async (linha) => {
  await props.controller?.removerLinha?.(linha)
}
...
const limparSuccess = () => {
  if (props.controller?.successMessage && typeof props.controller.successMessage === 'object' && 'value' in props.controller.successMessage) {
    props.controller.successMessage.value = null
  }
}
</script>

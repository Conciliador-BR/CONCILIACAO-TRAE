  ...
  <div class="rounded-2xl border border-gray-200/60 bg-white/70 backdrop-blur shadow-xl">
    <div class="px-6 py-4 border-b border-gray-200/70">
      <div class="flex items-center justify-between">
        <div>
          <h3 class="text-lg font-semibold text-gray-900">Análise Detalhada - {{ adquirente }}</h3>
          <p class="text-sm text-gray-600 mt-1">Relação completa de vendas, taxas e lucratividade do adquirente</p>
        </div>
        <div class="flex items-center space-x-3">
          <select v-model="filtroAtivo" class="text-sm border border-gray-300 rounded-md px-3 py-2 bg-white/90 backdrop-blur focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <option v-for="b in bandeirasUnicas" :key="b" :value="b">{{ b }}</option>
          </select>
...
const bandeirasUnicas = computed(() => [...new Set(props.dados.map(i => i.bandeira))].sort())
const dadosFiltrados = computed(() => filtroAtivo.value === 'todas' ? props.dados : props.dados.filter(i => i.bandeira === filtroAtivo.value))

const totalTransacoes = computed(() => dadosFiltrados.value.reduce((acc, i) => acc + (i.quantidade || 0), 0))
const totalReceitaBruta = computed(() => dadosFiltrados.value.reduce((acc, i) => acc + (i.receitaBruta || 0), 0))
const totalCustoTaxa = computed(() => dadosFiltrados.value.reduce((acc, i) => acc + (i.custoTaxa || 0), 0))
const totalReceitaLiquida = computed(() => dadosFiltrados.value.reduce((acc, i) => acc + (i.receitaLiquida || 0), 0))
...
const exportarDados = () => {
  ...
  link.href = url
  link.download = `analise-${props.adquirente}.csv`
  link.click()
